...
7. ⚠️ **禁止在結果欄空白時，拿參考範圍的數字去填 value**。
   例如看到「RBC　6.54 - 12.20」而**結果欄什麼都沒印**，就回傳 value: 6.54——這是嚴重錯誤，實測發生過。
   正確做法是完全不要回傳 RBC 這一項。
   ✅ **但請注意：判斷依據是「結果欄有沒有印數字」，不是「數字長得像不像參考範圍的邊界」。**
   如果結果欄確實印了數字，**即使那個數字剛好等於參考範圍的下界、上界或正中央，也必須照實回報，不可以因為「看起來可疑」就跳過**。
   例如「K　3.5 mmol/L　參考 3.5 - 5.8」——結果欄真的印了 3.5，這是真實數值，必須回報。
8. 若整張報告的結果欄都是空的（例如尚未填寫的空白檢驗單、日期還是 ____年__月__日 這種填空底線），
   items 必須回傳空陣列 []，且 resultColumnHasPrintedValues 必須為 false。
9. ⚠️ **沒有印參考範圍的列，一樣要抽出來**，不可以因為「這列沒有參考範圍」就當成不完整而跳過。
   檢驗報告常見的計算比值就屬於這類，例如 BUN/CREA、ALB/GLOB、Na/K、Osm Calc（估算滲透壓）——
   它們在結果欄有實際數字、只是沒有參考區間。這些列要照常放進 items，refRange 留空字串、status 回 "UNKNOWN"。
10. 計算 totalRowsWithValues 時，**上述沒有參考範圍的比值列也要一起數進去**，
   標準只有一個：「檢驗結果」欄位有沒有印出數字。
11. ⚠️ **有些報告每一列會印兩欄數值，是「本次結果」跟「上次/舊結果」的對照**（常見標示是兩個不同的採檢日期，例如「2026/07/07」跟「2025/10/16」並排，或欄位標題寫著「Current／Previous」「本次／前次」）。
   遇到這種雙欄對照格式：
   - **只抽取日期最新（本次）那一欄的數值**，另一欄（較舊的日期）完全忽略，不要放進 items。
   - 先找出哪一欄的日期最新，再逐列只讀那一欄——不要單看數字「看起來合理」就採用，兩欄的數字通常都落在正常範圍內，光看數字本身分辨不出來是哪一欄。
   - 這是目前最容易抓錯欄位的情境，寧可多花一點注意力確認欄位歸屬，也不要把舊欄位的數字當成本次結果。`;
