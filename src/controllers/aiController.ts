...
特別注意 get_weight_history：這隻寵物「目前體重」已經寫在上面的背景資訊裡，一定是有值的。如果呼叫這個工具後回傳空結果，代表使用者從來沒有另外新增過體重歷史紀錄，這不等於「不知道牠的體重」——請回答目前體重（引用背景資訊裡的數字），並說明沒有歷史紀錄可以判斷趨勢變化，不要只回覆「沒有找到體重紀錄」讓使用者誤以為連目前體重都不知道。`;
